...
class XNicer(BaseEstimator):
    ...
    # FIXME: Next procedure is the old one, to be removed!
    def predict(self, cols, use_classes=True, full=False, n_iters=3):
        """Compute the extinction for each object of a ColorCatalogue.
        ...
        if not cols.meta['use_projection']:
        ...
        log_weights0_ = - Tlogdet - \
            (cols.meta['n_bands'] - 1) * np.log(2.0*np.pi) / 2.0 - \
