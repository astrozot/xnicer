...
def generate_test_xdmix(xamp, xmean, xcovar, ycovar, npts=1000,
    ...
    """Create a full test for the XDGaussianMixture class.
    ...
    xdm = XDGaussianMixture(xamp.shape[0])  # pylint: disable=unsubscriptable-object
...
def test_xdmix_1d():
    ...
    for seed in range(3):
        ...
        a, b, c = generate_test_xdmix([0.25, 0.25, 0.5], [[0.0], [5.0], [-4.0]],
                                      [[[0.8]], [[1.2]], [[1.0]]], 0.5,
...
def test_xdmix_2d_projection():
    ...
    for seed in range(4):
        ...
        a, b, c = generate_test_xdmix([0.25, 0.75], [[3.0, 1.0], [-3.0, -1.0]],
                                      [1.0, 0.5], [1.0, 1.0],
                                      use_projection='random', npts=npts, seed=seed, silent=True)
        assert np.all(
            np.abs(a[1]) < 3), f"|a[1]| = {np.max(np.abs(a[1])):.2f} > 3"
